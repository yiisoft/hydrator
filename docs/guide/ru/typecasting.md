...
По умолчанию эти атрибуты не поддерживают многобайтовые кодировки, поэтому
символы юникод-пробелов, такие как `U+00A0` (неразрывный пробел) или
`U+2003` (широкий пробел), не удаляются. Чтобы удалять и их, включите
параметр `multibyte`:
...
Режим многобайтовой обработки использует функции `mb_trim()`, `mb_ltrim()` и
`mb_rtrim()`, которые предоставляются расширением PHP `mbstring`, начиная с
PHP 8.4. Чтобы использовать его на более ранних версиях PHP, установите
пакет [symfony/polyfill-mbstring](https://github.com/symfony/polyfill-mbstring).
Параметр `encoding` задаёт кодировку, используемую в многобайтовом режиме;
значение `null` (по умолчанию) означает использование `mb_internal_encoding()`.

С помощью `..` можно указать диапазон символов в параметре `characters`,
например, `а..я`. Это работает как в обычном, так и в многобайтовом режиме.
Этот синтаксис устарел и будет удалён в следующей мажорной версии, поэтому
...
- `separator` — символ перевода строки (по умолчанию, `\R`), это часть
  регулярного выражения, поэтому его следует учитывать или правильно
  экранировать с помощью `preg_quote()`;
- `multibyte` — использовать ли многобайтовую обработку при включённом
  `trim` (необязательное логическое значение, по умолчанию `null`, что
  означает использование значения по умолчанию из резолвера); требует
  функцию `mb_trim()`, предоставляемую расширением PHP `mbstring`, начиная
  с PHP 8.4, либо пакет
  [symfony/polyfill-mbstring](https://github.com/symfony/polyfill-mbstring)
  на более ранних версиях;
- `encoding` — кодировка, используемая в многобайтовом режиме (необязательная
  строка, по умолчанию `null`, что означает использование значения по
  умолчанию из резолвера).
